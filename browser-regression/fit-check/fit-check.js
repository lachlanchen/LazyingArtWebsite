(() => {
  ...
  const subject = "Browser Regression Baseline — free fit check";
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      site_url: clean(data.get("site_url")),
      flows: clean(data.get("flows")),
      environment: clean(data.get("environment")),
      instability: clean(data.get("instability")),
  ...
  const buildRequest = (payload) => {
    ...
    return [
      ...
      "",
      "Public site URL and authority:", payload.site_url,
      "",
      "Up to three priority flows:", payload.flows,
      "",
      "Repository and CI environment:", optional("environment"),
      "",
      "Known failures or unstable dependencies:", optional("instability"),
      "",
      "Preferred delivery and deadline:", payload.delivery,
      "",
      "Other constraints:", optional("constraints"),
      "",
      "Rights confirmation: I control this site or am authorized to request tests for its public flows.",
      "Scope confirmation: I understand the fixed USD 250 baseline covers one public website, up to three agreed flows and twelve checkpoints, Chromium at 1440 × 1000 and 390 × 844, and one correction pass. Login, private staging, CAPTCHA or MFA bypass, destructive or payment actions, cross-browser coverage, site fixes, certification, deployment, and ongoing maintenance are excluded.",
