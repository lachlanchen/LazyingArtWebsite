...
test("video portfolio has one canonical direct-view page", () => {
  ...
  assert.match(html, /brand-film\/sample\/madeira-six-clip-master\.mp4/);
});

test("video portfolio states roles and keeps claims bounded", () => {
  assert.equal((html.match(/<dt>Role<\/dt>/g) || []).length, 5);
  assert.equal((html.match(/<dt>Constraint<\/dt>/g) || []).length, 5);
