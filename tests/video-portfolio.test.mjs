...
test("video portfolio has one canonical direct-view page", () => {
  ...
  assert.doesNotMatch(html, /youtube-nocookie\.com\/embed\/rVU37lPKPo8/);
  assert.match(html, /story-clip\/sample\/selected-provenance-clip\.mp4/);
  assert.doesNotMatch(html, /brand-film\/sample\/madeira-six-clip-master\.mp4/);
  assert.doesNotMatch(html, /LalaMedias\/videos\/aginti-autonomous-lab-ai-glasses/);
  assert.match(html, /\.\.\/lecture-pack\/#example/);
});

test("video portfolio states roles and keeps claims bounded", () => {
  assert.equal((html.match(/<dt>Role<\/dt>/g) || []).length, 3);
  assert.equal((html.match(/<dt>Constraint<\/dt>/g) || []).length, 3);
