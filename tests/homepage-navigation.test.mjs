...
test('navigation retains all thirteen destinations and labels its disclosure in every locale', () => {
  const nav = html.match(/<div class="nav-links" id="navLinks">([\s\S]*?)<\/div>/)[1];
  const hrefs = [...nav.matchAll(/href="([^"]+)"/g)].map((match) => match[1]);
  assert.deepEqual(hrefs, ['#services', 'work/', '#ecosystem', '#coin', '#product',
