...
assert.doesNotMatch(fitCheck, /data-testid="send-fit-check" type="button" disabled hidden/);
assert.match(fitCheck, /After reviewing, choose “Send fit check,” “Open in email,” or “Copy request\.”/);
assert.match(fitCheck, /“Send fit check” sends this request to LazyingArt\./);
assert.match(fitCheck, /“Open in email” prepares it in your email app without sending it\./);
assert.match(fitCheck, /“Copy request” copies the same text\./);
