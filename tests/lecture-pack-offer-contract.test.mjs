...
assert.match(offer, /Aligned segments<\/strong><span>English and Traditional Chinese<\/span><b>8<\/b>/);
assert.match(offer, /ASR corrections<\/strong><span>Each change recorded<\/span><b>5<\/b>/);
assert.match(offer, /Unresolved English words<\/strong><span>This synthetic sample<\/span><b>0<\/b>/);
assert.match(offer, /bilingual-caption-preview\.mp4/);
assert.match(offer, /bilingual-lecture-pack-sample\.zip/);
assert.match(offer, /bilingual-lecture-pack-sample\.zip\.sha256/);
assert.match(offer, /ab36ac63c57f7acbd9d82238fdbf0c80c0349c6c/);
assert.match(offer, /translation is assisted and terminology-checked, not certified/i);
...
assert.match(fitCheck, /send the encrypted request here or continue with your own email app/i);
assert.match(
  fitCheck,
  /Review the request, then send it encrypted or use your own email app\. No source upload or payment\./,
);
...
assert.match(fitCheck, /send the encrypted request here/i);
assert.match(fitCheck, /encrypts the reviewed request before it is stored for review/i);
assert.doesNotMatch(
  fitCheck,
  /data-testid="send-fit-check" type="button" disabled hidden/,
);
