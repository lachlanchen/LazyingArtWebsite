...
assert.match(offer, /<meta name="robots" content="noindex,follow">/);
assert.match(offer, /AI Clip Assembly Archive/);
assert.match(offer, /This pilot is paused/);
assert.match(offer, /does not meet the current visual-quality bar/);
assert.doesNotMatch(offer, /"@type": "Service"/);
assert.doesNotMatch(offer, /USD 500/);
assert.match(offer, /Up to six customer-controlled source clips, up to 45 seconds combined/i);
...
assert.match(offer, /Native Premiere Pro, After Effects, Final Cut, or DaVinci Resolve project delivery was never implied/i);
assert.match(offer, /Customer footage is not reused as public proof without separate permission/i);
assert.doesNotMatch(offer, /href="fit-check\/"/);
...
assert.doesNotMatch(portfolio, /href="brand-film\/">See the USD 500 assembly pilot/);
assert.match(portfolio, /Three finished pieces/);
assert.doesNotMatch(portfolio, /brand-film\/sample\/madeira-six-clip-master\.mp4/);
assert.doesNotMatch(portfolio, /href="brand-film\/#sample">Watch both cuts and inspect the build/);
assert.doesNotMatch(sitemap, /<loc>https:\/\/lazying\.art\/video\/brand-film\/<\/loc>/);
assert.doesNotMatch(sitemap, /<loc>https:\/\/lazying\.art\/video\/brand-film\/fit-check\/<\/loc>/);
