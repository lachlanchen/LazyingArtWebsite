(() => {
  ...
  const subject = "Pronunciation mini-lesson — free fit check";
  ...
  const buildPayload = () => {
    ...
    return {
      ...
      contrast: clean(data.get("contrast")),
      learner_context: clean(data.get("learner_context")),
      language: clean(data.get("language")),
      intended_use: clean(data.get("intended_use")),
  ...
  const buildRequest = (payload) => {
    ...
    return [
      ...
      "",
      "Sound contrast or word pair:", payload.contrast,
      "",
      "Learner group:", payload.learner_context,
      "",
      "Bilingual lesson language:", payload.language,
      "",
      "Use and teaching outcome:", payload.intended_use,
      "",
      "Optional source or brand material:", optional("source"),
      "",
      "Desired date and deployment plan:", payload.deadline,
      "",
      "Other constraints:", optional("constraints"),
      "",
      "Rights and privacy confirmation: I am authorized to request this work and will not send learner recordings, personal data, credentials, or files before acceptance.",
      "Scope confirmation: I understand the fixed USD 250 lesson and its exclusions.",
